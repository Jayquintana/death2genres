...
BEGIN
  ...
  DROP POLICY IF EXISTS "Authenticated users can manage playlists" ON playlists;
END $$;
...
-- Insert initial playlist if it doesn't exist
...
SELECT
  ...
  0
WHERE NOT EXISTS (
  SELECT 1 FROM playlists WHERE spotify_url = 'https://open.spotify.com/playlist/7dAYXEa5eHX8EVII4NosEE'
);